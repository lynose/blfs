...
if test -d /sources/rustc-1.52.0-src
 then
  as_root rm -rf /sources/rustc-1.52.0-src
fi

SCRIPT=`realpath $0`
SCRIPTPATH=`dirname $SCRIPT`

check_and_download https://static.rust-lang.org/dist/rustc-1.52.0-src.tar.gz \
    /sources

md5sum -c ${SCRIPTPATH}/md5-rustc &&

tar xf /sources/rustc-1.52.0-src.tar.gz -C /sources/ &&

cd /sources/rustc-1.52.0-src &&

as_root mkdir -p /opt/rustc-1.52.0             &&
if [ -L /opt/rustc ];
 then 
  as_root rm /opt/rustc
fi
as_root ln -svfin rustc-1.52.0 /opt/rustc &&
