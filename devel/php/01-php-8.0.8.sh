...
if test -d /sources/php-8.0.8
 then
  rm -rf /sources/php-8.0.8
fi

SCRIPT=`realpath $0`
SCRIPTPATH=`dirname $SCRIPT`

check_and_download http://www.php.net/distributions/php-8.0.8.tar.xz \
        /sources &&
...
tar xf /sources/php-8.0.8.tar.xz -C /sources/ &&

cd /sources/php-8.0.8 &&
...
if [ ! -d /usr/share/doc/php-8.0.8 ]
 then
  as_root install -v -m755 -d /usr/share/doc/php-8.0.8
fi
as_root install -v -m644    CODING_STANDARDS* EXTENSIONS NEWS README* UPGRADING* \
                    /usr/share/doc/php-8.0.8 &&
...
as_root tar -xvf ../php_manual_en.tar.gz \
    -C /usr/share/doc/php-8.0.8 --no-same-owner &&
cp /etc/php.ini /tmp &&
