...
if test -d /sources/Python-3.9.4
 then
  as_root rm -rf /sources/Python-3.9.4
fi

SCRIPT=`realpath $0`
SCRIPTPATH=`dirname $SCRIPT`

check_and_download https://www.python.org/ftp/python/3.9.4/Python-3.9.4.tar.xz \
    /sources &&

md5sum -c ${SCRIPTPATH}/md5-python3 &&

tar xf /sources/Python-3.9.4.tar.xz -C /sources/ &&
...
as_root ln -svfn python-3.9.4 /usr/share/doc/python-3 &&
