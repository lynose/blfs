...
if test -d /sources/qemu-5.1.0
 then
  rm -rf /sources/qemu-5.1.0
fi

SCRIPT=`realpath $0`
SCRIPTPATH=`dirname $SCRIPT`

check_and_download http://download.qemu-project.org/qemu-5.1.0.tar.xz \
        /sources


md5sum -c ${SCRIPTPATH}/md5-qemu &&

tar xf /sources/qemu-5.1.0.tar.xz -C /sources/ &&

cd /sources/qemu-5.1.0 &&
