...
if test -d /sources/openldap-2.5.5
 then
  rm -rf /sources/openldap-2.5.5
fi

SCRIPT=`realpath $0`
SCRIPTPATH=`dirname $SCRIPT`

check_and_download https://www.openldap.org/software/download/OpenLDAP/openldap-release/openldap-2.5.5.tgz \
        /sources

check_and_download http://www.linuxfromscratch.org/patches/blfs/svn/openldap-2.5.5-consolidated-1.patch \
        /sources        

md5sum -c ${SCRIPTPATH}/md5-openldap &&

tar xf /sources/openldap-2.5.5.tgz -C /sources/ &&

cd /sources/openldap-2.5.5 &&
...
patch -Np1 -i ../openldap-2.5.5-consolidated-1.patch &&
autoconf &&
...
as_root install -v -dm755 /usr/share/doc/openldap-2.5.5 &&
as_root cp      -vfr      doc/{drafts,rfc,guide} \
                  /usr/share/doc/openldap-2.5.5 &&
