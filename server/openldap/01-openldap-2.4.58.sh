...
if test -d /sources/openldap-2.4.58
 then
  rm -rf /sources/openldap-2.4.58
fi

SCRIPT=`realpath $0`
SCRIPTPATH=`dirname $SCRIPT`

check_and_download https://www.openldap.org/software/download/OpenLDAP/openldap-release/openldap-2.4.58.tgz \
        /sources

check_and_download http://www.linuxfromscratch.org/patches/blfs/svn/openldap-2.4.58-consolidated-1.patch \
        /sources        

md5sum -c ${SCRIPTPATH}/md5-openldap &&

tar xf /sources/openldap-2.4.58.tgz -C /sources/ &&

cd /sources/openldap-2.4.58 &&
...
as_root install -v -dm755 /usr/share/doc/openldap-2.4.58 &&
as_root cp      -vfr      doc/{drafts,rfc,guide} \
                  /usr/share/doc/openldap-2.4.58 &&
