...
if test -d /sources/cups-filters-1.28.7
 then
  rm -rf /sources/cups-filters-1.28.7
fi

SCRIPT=`realpath $0`
SCRIPTPATH=`dirname $SCRIPT`

check_and_download https://www.openprinting.org/download/cups-filters/cups-filters-1.28.7.tar.xz \
    /sources &&

md5sum -c ${SCRIPTPATH}/md5-cups-filters &&

tar xf /sources/cups-filters-1.28.7.tar.xz -C /sources/ &&

cd /sources/cups-filters-1.28.7 &&
