...
if test -d /sources/ghostscript-9.53.3
 then
  rm -rf /sources/ghostscript-9.53.3
fi
...
check_and_download https://github.com/ArtifexSoftware/ghostpdl-downloads/releases/download/gs9533/ghostscript-9.53.3.tar.xz \
    /sources
...
check_and_download https://downloads.sourceforge.net/gs-fonts/gnu-gs-fonts-other-6.0.tar.gz \
    /sources
check_and_download http://www.linuxfromscratch.org/patches/blfs/svn/ghostscript-9.53.3-freetype_fix-1.patch \
    /sources
    
md5sum -c ${SCRIPTPATH}/md5-ghostscript &&

tar xf /sources/ghostscript-9.53.3.tar.xz -C /sources/ &&

cd /sources/ghostscript-9.53.3 &&
...
if [  ! -d /usr/share/doc/ghostscript/9.53.3 ]
 then
  as_root mv -v /usr/share/doc/ghostscript/9.53.3 /usr/share/doc/ghostscript-9.53.3  &&
  as_root cp -r examples/ /usr/share/ghostscript/9.53.3/
fi
